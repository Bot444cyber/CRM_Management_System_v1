...
async function migrate() {
    ...
    const [inventoryCols] = await db_1.db.execute((0, drizzle_orm_1.sql) `
    ...
    if (inventoryCols.length === 0) {
        await db_1.db.execute((0, drizzle_orm_1.sql) `ALTER TABLE inventories ADD COLUMN user_id INT NULL AFTER id`);
    ...
    const [analyticsCols] = await db_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'analytics'
        AND COLUMN_NAME = 'user_id'
    `);
    if (analyticsCols.length === 0) {
        await db_1.db.execute((0, drizzle_orm_1.sql) `ALTER TABLE analytics ADD COLUMN user_id INT NULL AFTER id`);
    ...
    const [activityTable] = await db_1.db.execute((0, drizzle_orm_1.sql) `
