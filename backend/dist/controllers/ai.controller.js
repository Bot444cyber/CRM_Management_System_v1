...
exports.chatWithProductAssistant = exports.extractProductDetails = void 0;
const genai_1 = require("@google/genai");
const db_1 = require("../config/db");
const schema_1 = require("../db/schema");
const drizzle_orm_1 = require("drizzle-orm");
const extractProductDetails = async (req, res) => {
    try {
...
const chatWithProductAssistant = async (req, res) => {
    try {
        ...
        try {
            const salesLogs = await db_1.db.select().from(schema_1.activityLogs)
        ...
        const catalogDetails = catalog.map((p) => `- ${p.name} (in ${p.inventory} / ${p.mainProduct}) | Price: $${p.price} | Stock: ${p.stock > 0 ? p.stock : 'Out of stock'} | Status: ${p.status}`).join('\n');
...
exports.chatWithProductAssistant = chatWithProductAssistant;
